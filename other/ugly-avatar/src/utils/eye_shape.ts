import { randomFromInterval, cubicBezier } from '.'
...
export function generateEyePoints(rands: EyeParameters, width = 50): EyePoints {
	let P0_upper = [
	...
	let upper_eyelid_points: number[][] = []
	let upper_eyelid_points_left_control: number[][] = []
	let upper_eyelid_points_right_control: number[][] = []
	...
	// now we generate the points for the upper eyelid
	let lower_eyelid_points: number[][] = []
	let lower_eyelid_points_left_control: number[][] = []
	let lower_eyelid_points_right_control: number[][] = []
...
export function generateBothEyes(width = 50) {
	...
	for (let key in rands_right) {
		...
		if (typeof rands_right[key as keyof EyeParameters] === 'number') {
			// Add a random value to the number, for example, between -5 and 5
			rands_right[key as keyof EyeParameters] += randomFromInterval(
				-rands_right[key as keyof EyeParameters] / 2.0,
				rands_right[key as keyof EyeParameters] / 2.0,
			)
		}
	}
